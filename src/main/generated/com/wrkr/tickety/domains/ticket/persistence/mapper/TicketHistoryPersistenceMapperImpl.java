...
@Component
public class TicketHistoryPersistenceMapperImpl implements TicketHistoryPersistenceMapper {

    @Override
    public TicketHistoryEntity toEntity(TicketHistory domain) {
        if ( domain == null ) {
            return null;
        }

        TicketHistoryEntity.TicketHistoryEntityBuilder<?, ?> ticketHistoryEntity = TicketHistoryEntity.builder();

        ticketHistoryEntity.createdAt( domain.getCreatedAt() );
        ticketHistoryEntity.updatedAt( domain.getUpdatedAt() );
        ticketHistoryEntity.ticketHistoryId( domain.getTicketHistoryId() );
        ticketHistoryEntity.ticket( ticketToTicketEntity( domain.getTicket() ) );
        ticketHistoryEntity.manager( memberToMemberEntity( domain.getManager() ) );
        ticketHistoryEntity.status( domain.getStatus() );
        ticketHistoryEntity.modified( domain.getModified() );

        return ticketHistoryEntity.build();
    }

    @Override
    public TicketHistory toDomain(TicketHistoryEntity entity) {
        if ( entity == null ) {
            return null;
        }

        TicketHistory.TicketHistoryBuilder<?, ?> ticketHistory = TicketHistory.builder();

        ticketHistory.createdAt( entity.getCreatedAt() );
        ticketHistory.updatedAt( entity.getUpdatedAt() );
        ticketHistory.ticketHistoryId( entity.getTicketHistoryId() );
        ticketHistory.ticket( ticketEntityToTicket( entity.getTicket() ) );
        ticketHistory.manager( memberEntityToMember( entity.getManager() ) );
        ticketHistory.status( entity.getStatus() );
        ticketHistory.modified( entity.getModified() );

        return ticketHistory.build();
    }
