...
//    @CustomErrorCodes({MemberErrorCode.MEMBER_NOT_FOUND})
    ...
    @Operation(summary = "관리자 - 회원 정보 목록 조회", description = "전체 회원을 페이징으로 조회합니다. page는 1 이상, size는 10 이상이어야 합니다")
    @GetMapping
    public ApplicationResponse<MemberInfoPagingResponse> getTotalMemberInfo(@RequestParam(defaultValue = "0") @Min(value = 1, message = "페이지 번호는 1 이상이어야 합니다.") int page,
                                                                            @RequestParam(defaultValue = "10") @Min(value = 10, message = "페이지 크기는 10 이상이어야 합니다.") int size,
                                                                            @RequestParam(required = false) String role) {
        return ApplicationResponse.onSuccess(totalMemberInfoGetUseCase.getTotalMemberInfo(page - 1, size, role));
    }
}
